...
export default function AdminCocktailsPage() {
  ...
  useEffect(() => {
    getJSON<Row[]>("/api/admin/cocktails")
      .then(setRows)
  ...
  async function remove(id: string) {
    if (!confirm("确定删除这款酒谱？")) return;
    try {
      await sendJSON(`/api/admin/cocktails/${id}`, "DELETE");
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-ink">酒谱管理</h1>
        <Link
      ...
      {loading ? (
        ...
        <div className="mt-5 divide-y divide-border rounded-2xl border border-border bg-card">
          {rows.map((r) => (
            <div key={r.id} className="flex items-center justify-between px-4 py-3">
              ...
              <div className="flex gap-1">
                <Link
                  href={`/admin/cocktails/${r.id}/edit`}
          ...
          {rows.length === 0 && (
            <p className="py-8 text-center text-sm text-muted">还没有酒谱</p>
          )}
        </div>
      )}
    </div>
